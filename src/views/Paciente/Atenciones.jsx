...
export default function Atenciones({ navigation, route }) {
    ...
    const deleteAtencion = (idVacuna) => {
        ...
        deleteDoc(doc(db, "patients", id, "atenciones", idVacuna))
            ...
            .catch((error) => {
                setUploading(false);
                Alert.alert("Error", "Ocurrio un error al intentar eliminar la atencion");
            });
    };
    ...
    useFocusEffect(
        React.useCallback(() => {
            ...
            const unsuscribe = onSnapshot(q,
                (atenciones) => {
                    let listadoAtenciones = [];
                    atenciones.forEach((doc) => {
                        console.log(doc.data());
                        const { descripcion, fecha, proximaCita, tipo } =
                            doc.data();
                ...
                <ScrollView style={uploading ? { opacity: 0.5 } : { opacity: 1 }}>
                    ...
                    <Box flex={1} p={1} w="95%" mx="auto">
                        <VStack mt={15} mb={15} space={2} px="2">
                            <Heading size="md" marginY={1} bold alignSelf={'center'}>Menu Atenciones</Heading>
                                            ...
                                            <ListItem.Content>
                                                ...
                                                <View>
                                                    <Text style={styles.ratingText}>
                                                        Fecha Atencion: {timestampToDate(atencion.fecha.seconds)}
                                                    </Text>
                                                </View>
                                            </ListItem.Content>
                                            <ListItem.Chevron />
